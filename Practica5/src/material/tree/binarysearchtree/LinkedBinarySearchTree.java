...
//    }
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) throws RuntimeException{
    	...
    	int comp2;
    	while (it.hasNext()) {
    		aux = (Position<E>) it.next();
    		comp1 = this.comparator.compare(minValue, aux.getElement());
    		comp2 = this.comparator.compare(maxValue, aux.getElement());
    ...
    public Position<E> first() throws RuntimeException { 
    	...
    	while (this.binTree.hasLeft(aux)) {
    ...
    public Position<E> last() throws RuntimeException { 
    	Position<E> aux = this.binTree.root();
    	if(!this.binTree.hasRight(aux)) {
    		throw new RuntimeException("No last element.");
    	}
        while(this.binTree.hasRight(aux)){
    ...
    public Iterable<Position<E>> successors(Position<E> pos){ 
    	...
    	while (it.hasNext()) {
    		aux = (Position<E>) it.next();
    		comp = this.comparator.compare(aux.getElement(), pos.getElement());
    		if(comp >= 0){
    			List.add(aux);
    		}
    	}
...
class ReestructurableBinaryTree<T> extends LinkedBinaryTree<T>{
	...
	public Position<T> restructure(Position<T> posNode, LinkedBinarySearchTree<T> bst) {
		...
		if (bst.binTree.isRoot(posGrandParent)) {
                        bst.binTree = (LinkedBinaryTree<T>) bst.binTree.subTree(midKey);
		} else {
